...
function usage() {
	console.log("Usage:\n\t" + process.argv[1].split("/").pop() + " <register|unregister> <guest>")
}
...
function success(result) {
  log(context.deskOp + " success for guest " + this.guestName);
  log("Transaction id: " + result.result.message);
}

function failure(result) {
  log(context.deskOp + " failure for guest " + this.guestName);
